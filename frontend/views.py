...
def profile(request):
    ...
    if request.method == 'POST':
        ...
        if 'profile_picture' in request.FILES:
            ...
            logger.debug(f"Sending file: {files['profile_picture'].name}, size: {files['profile_picture'].size}")
        else:
            logger.warning("No profile_picture found in request.FILES")

        try:
            response = requests.put(f'{API_BASE_URL}profile/update/', headers=headers, data=form_data, files=files)
            ...
            messages.error(request, f'Có lỗi xảy ra. Vui lòng thử lại sau: {str(e)}')
...
def register_driver(request):
    ...
    if hasattr(request.user, 'driverprofile') and request.user.driverprofile is not None:
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Tạo DriverProfile với các trường hợp lệ
            DriverProfile.objects.create(
                ...
                brand=brand,
                model=model,
                year=int(year),  # Chuyển đổi sang integer
